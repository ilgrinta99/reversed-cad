...
Nessuna dipendenza esterna: il PDF e il DXF sono scritti a mano. Nemmeno FreeCAD:
questo modulo gira sotto qualunque interprete, ed e' il motivo per cui la tavola
si puo' comporre fuori dal processo FreeCAD che ha proiettato la geometria.

Sta in `core/` perche' non contiene un solo numero di un pezzo: quello che si
disegna lo decide chi chiama (`core/drafting/tavola.py` e i plugin di `parts/`).
Era `cad/draft2d.py`, che ora e' un guscio che reimporta da qui.
"""
import math, zlib
...
class Foglio(object):
    ...
    def richiamo(self, punto, testo_xy, righe, h=2.4, st='quota'):
        """Linea di richiamo da `punto` a `testo_xy`, con una o piu' righe di testo.

        Il tratto orizzontale sotto il testo e' la convenzione ISO 129: la freccia
        tocca la feature, il testo sta all'estremita' del ripiano.
        """
        px, py = punto
        tx, ty = testo_xy
        verso = 1.0 if tx >= px else -1.0
        ripiano = 3.0 * verso
        self.linea(px, py, tx - ripiano, ty, st)
        self.linea(tx - ripiano, ty, tx, ty, st)
        d = math.hypot(tx - ripiano - px, ty - py)
        if d > 1e-9:
            self._freccia((px, py), ((tx - ripiano - px) / d, (ty - py) / d))
        anc = 'start' if verso > 0 else 'end'
        for i, riga in enumerate(righe):
            self.testo(tx + 0.8 * verso, ty + 1.0 - (h + 0.9) * i, riga, h, anc=anc, st=st)

    def riquadro(self, x, y, w, h, titolo=None, st='sottile'):
        """Cornice rettangolare con titolo in grassetto, per note e tabelle."""
        self.polilinea([(x, y), (x + w, y), (x + w, y + h), (x, y + h)], st, chiusa=True)
        if titolo:
            self.testo(x + 3.0, y + h - 6.0, titolo, 3.0, st='contorno', grassetto=True)

    def tabella(self, x, y, w, titolo, intestazioni, righe, h_riga=5.0, h_testo=2.3):
        """Tabella a colonne allineate. `intestazioni` = [(etichetta, x_rel, anc)].

        Restituisce l'altezza occupata: chi impagina decide dove mettere il resto.
        """
        h = 13.0 + h_riga * (len(righe) + 1)
        self.riquadro(x, y, w, h, titolo)
        yy = y + h - 13.0
        for etichetta, xr, anc in intestazioni:
            self.testo(x + xr, yy, etichetta, 2.2, st='contorno', anc=anc)
        self.linea(x + 2.0, yy - 2.0, x + w - 2.0, yy - 2.0, 'sottile')
        for i, riga in enumerate(righe):
            yr = yy - h_riga * (i + 1)
            for (_, xr, anc), (testo, stile) in zip(intestazioni, riga):
                self.testo(x + xr, yr, testo, h_testo, st=stile, anc=anc)
        return h
...
# ------------------------------------------------------------- testo nel PDF
# I font base del PDF sono a un byte e qui sono dichiarati /WinAnsiEncoding, che
# e' cp1252: quello, e non latin-1, e' il gioco di caratteri da usare per scrivere.
# Cambia poco per l'italiano e molto per la punteggiatura tipografica — trattino
# lungo, virgolette, apici — che in latin-1 diventerebbe '?'.
# Quel che non c'e' nemmeno in cp1252 si translittera invece di sparire.
TRANSLITTERA = {
    u'↔': '<->', u'→': '->', u'←': '<-',
    u'≤': '<=', u'≥': '>=', u'≠': '!=',
    u'±': '+/-', u'≈': '~', u'×': 'x',
...
def scrivi_pdf(fogli, path, titolo='Disegno'):
    ...
    for f in fogli:
        ...
        for pr in f.p:
            ...
            if pr[0] == 'line':
                ...
                c.append('h f')
                cur = None
        pagine.append((f, zlib.compress(_cp1252('\n'.join(c)))))
    ...
    info = add(_cp1252('<< /Title (%s) /Producer (core.drafting) >>' % titolo))
